...
| Phone OTP login | **Done** | `/login` + `LoginModal` — send/verify OTP, complete profile name |
| Email login | **Done** | Same flow; seed password `Customer@123` for sample emails |
...
| Address modal | **Done** | `AddressModal` — map/search/GPS + Save as Home/Work/Hotel/Other + form fields + Save Address |
...
| `/` | Home shell |
| `/login` | Auth |
| `/account` | Account hub (Your Information) |
